...
const Dashboard = () => {
  ...
  const partialPaymentSales = sales.filter(sale => sale.status === 'Partial Payment' || (sale as any).payment_method === 'partial');
  ...
  const todaysSales = sales.filter(sale => sale.sale_date === today).reduce((total, sale) => total + Number(sale.total_amount), 0);
  ...
  const customersWithDebt = customers.filter(customer => {
    const customerSales = sales.filter(sale => sale.customer_name === customer.name && (sale.status === 'Partial Payment' || (sale as any).payment_method === 'partial'));
  ...
  return <div className="space-y-6 rounded-full bg-slate-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, index) => <Card key={index} className="p-6 hover:shadow-lg transition-all duration-200 border-0 shadow-md bg-white">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">{stat.title}</p>
                <p className="text-2xl font-bold text-gray-900">{stat.value}</p>
                <p className={`text-sm ${stat.title === 'Partial Payments' ? 'text-purple-600' : stat.change.startsWith('+') ? 'text-green-600' : 'text-red-600'}`}>
                  {stat.change}
                </p>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="p-6 shadow-md hover:shadow-lg transition-all duration-200 bg-stone-200">
          ...
          <div className="space-y-4">
            {recentSales.length > 0 ? recentSales.map(sale => <div key={sale.id} className="flex items-center justify-between p-3 bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg">
                <div>
                  <p className="font-medium">{sale.order_id}</p>
                  <div className="flex items-center space-x-2">
                    <p className="text-sm text-gray-500">{sale.customer_name}</p>
                    {(sale.status === 'Partial Payment' || (sale as any).payment_method === 'partial') && <span className="px-2 py-1 bg-gradient-to-r from-orange-100 to-yellow-100 text-orange-800 text-xs rounded-full border border-orange-200">
                        Partial
                      </span>}
                  </div>
                </div>
              ...
              </div>) : <p className="text-gray-500">No sales yet</p>}
          </div>
        </Card>
        ...
        <Card className="p-6 shadow-md hover:shadow-lg transition-all duration-200 bg-zinc-100">
          ...
          <div className="space-y-4">
            ...
            const customerPartialSales = sales.filter(sale => sale.customer_name === customer.name && (sale.status === 'Partial Payment' || (sale as any).payment_method === 'partial')).length;
            return <div key={customer.id} className="flex items-center justify-between p-3 bg-gradient-to-r from-orange-50 to-red-50 rounded-lg border border-orange-200">
      ...
      <Card className="p-6 shadow-md hover:shadow-lg transition-all duration-200 bg-violet-50">
        ...
        <div className="space-y-4">
          {lowStockAlerts.length > 0 ? lowStockAlerts.map(product => <div key={product.id} className="flex items-center justify-between p-3 bg-gradient-to-r from-red-50 to-pink-50 rounded-lg border border-red-200">
              <div className="flex items-center space-x-3">
            ...
            </div>) : <p className="text-gray-500">No low stock items</p>}
        </div>
      </Card>
    </div>;
};
